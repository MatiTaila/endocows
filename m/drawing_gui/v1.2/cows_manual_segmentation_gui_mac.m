function varargout = cows_manual_segmentation_gui_mac(varargin)
% COWS_MANUAL_SEGMENTATION_GUI_MAC MATLAB code for cows_manual_segmentation_gui_mac.fig
%      COWS_MANUAL_SEGMENTATION_GUI_MAC, by itself, creates a new COWS_MANUAL_SEGMENTATION_GUI_MAC or raises the existing
%      singleton*.
%
%      H = COWS_MANUAL_SEGMENTATION_GUI_MAC returns the handle to a new COWS_MANUAL_SEGMENTATION_GUI_MAC or the handle to
%      the existing singleton*.
%
%      COWS_MANUAL_SEGMENTATION_GUI_MAC('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in COWS_MANUAL_SEGMENTATION_GUI_MAC.M with the given input arguments.
%
%      COWS_MANUAL_SEGMENTATION_GUI_MAC('Property','Value',...) creates a new COWS_MANUAL_SEGMENTATION_GUI_MAC or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before cows_manual_segmentation_gui_mac_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to cows_manual_segmentation_gui_mac_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help cows_manual_segmentation_gui_mac

% Last Modified by GUIDE v2.5 17-Jun-2013 11:33:07

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @cows_manual_segmentation_gui_mac_OpeningFcn, ...
                   'gui_OutputFcn',  @cows_manual_segmentation_gui_mac_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before cows_manual_segmentation_gui_mac is made visible.
function cows_manual_segmentation_gui_mac_OpeningFcn(hObject, eventdata, handles, varargin)
...
% varargin   command line arguments to cows_manual_segmentation_gui_mac (see VARARGIN)

% Choose default command line output for cows_manual_segmentation_gui_mac
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes cows_manual_segmentation_gui_mac wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = cows_manual_segmentation_gui_mac_OutputFcn(hObject, eventdata, handles) 
...
[points, h_curve, h_points] = cows_draw_closed_curve_mac(ud.n_curves);
